//Poner ganador, salir en cualquier momento y mostrar historial en cualquier momento.
package src.edd;
...
public class Wizard {
                ...
                do {
                    ...
                    try {
                        ...
                        if (numJugadores < 3 || numJugadores > 6) {
                            System.out.println("\u001B[33m"+"No es un numero valido de jugadores"+"\u001B[0m");
                        } else {
                            incorrecto = false;
                        }
                    } catch (InputMismatchException e) {
    ...
    public static void jugar(int numJugadores, int numRondas, Lista<Carta[][]> cartasPorRonda, Registro registro) {
        ...
        while (contador < numRondas) {
            ...
            for (int i = 0; i < jugadores.size(); i++) {
                ...
                while (k) {
                    ...
                    try {
                        apuesta = sc.nextInt();
                        verificaRespuesta(apuesta, contador,registro);////////////////////////////////////////////////////////////////////////////////////
                        if (apuesta > contador + 1) {
                            System.out.println("\u001B[33m"+"Numero no valido para apostar en esta ronda"+"\u001B[0m");
                            // sc.next();
            ...
            registro.getGanadoresDeTrucos().add(truco(contador+1, baraja, jugadores, paloTriunfo, barajeador, registro));
            it=jugadores.iterator();
        ...
        if(empate.isEmpty()){
            System.out.println("\u001B[32m"+"********* Ganador del juego: " + ganador.getNombre()+" **********"+"\u001B[0m");
        }else{
            it=jugadores.iterator();//reiniciamos el iterador
            System.out.print("\u001B[32m"+"********* Empate entre ");
            for(int i=0; i<empate.size();i++){
                System.out.print(it.next().getNombre()+" ");
            }
            System.out.print("**********"+"\u001B[0m");
        }
        historial(numRondas, registro);
    ...
    public static Cola<Jugador> truco(int numTrucos, Baraja baraja, Lista<Jugador> jugadores, String paloTriunfo,
        ...
        Carta[][] arrRegistro=new Carta[numTrucos][jugadores.size()];//Arreglo para ir poniendo las cartas que tira cada jugador en cada truco
        ...
        while (contador <= numTrucos) {
            ...
            while (i < jugadores.size()) {
                ...
                while (k) {
                    ...
                    try {
                        respuesta = sc.nextInt();
                        verificaRespuesta(respuesta, contador-1, registro);/////////////////////////////////////////////////////////////////////////////////////
                        if (respuesta < 0 || respuesta > (actual.getCartas().size() - 1)) {
                            if(respuesta!=-1&&respuesta!=-2){//Para verificar que en la respuesta no hayan solicitado el historial
                            ...
                            if(!paloLider.equals("")){
                                if(ayuda.getNumero()!=14&&ayuda.getNumero()!=0){
                                    if(!ayuda.getPalo().equals(paloLider)){
                                        ...
                                        if(nohay){
                                            arrRegistro[contador-1][actual.getNumJugador()]=actual.getCartas().elementoEnPos(respuesta);//Ponemos la carta tirada en el lugar correspondiente del arreglo para el registro
                                            actual.getCartasTiradas().push(actual.getCartas().eliminaEnPos(respuesta));
                                            k=false;
                                        }else{
                                            System.out.println("\u001B[36m"+"Debes tirar una carta del palo lider"+"\u001B[0m ");
                                            //\u001B[0m reset
                                            //\033[30m
                                            //\u001B[37m
                                        }
                ...
                if(ganador==null){//Si todavia no hay ganador
                    ...
                    if(numCartaGanador!=14){//Si nadie ha tirado un mago
                        ...
                        if(numCartaJugador==14){//Si la carta del jugador actual es mago
                            ...
                            if(paloGanador.equals(paloTriunfo)){//y la carta ganadora también coincide con el palo de triunfo
                        ...
                        }else if((numCartaGanador!=0||numCartaJugador!=0)&&!paloGanador.equals(paloTriunfo)&&!paloGanador.equals(paloLider)){//Entra aquí si son numeros y no son bufones
                            if(numCartaGanador<numCartaJugador){//Se comparan los numeros
                                ganador=actual;
                            }
                        }else{}//Si son bufones entonces el que lo tiro primero se queda como ganador
            ...
            System.out.println("\u001B[32m"+"### El ganador del truco es "+ ganador.getNombre()+" ###"+"\u001B[0m");
     ...
     * @param registro
     */
    public static void historial(int numRondas, Registro registro){
        System.err.println("\n&&&&&&&&&&&&&&&&&&&&&&&&&& Historial &&&&&&&&&&&&&&&&&&&&&&&&&&&&&&\n");
        ...
        for(int i=0;i<numRondas;i++){
            ...
            for(int j=0;j<aux.length;j++){//numero de trucos
                ...
                for(int k=0; k<aux[0].length;k++){
                    System.out.println("Jugador "+ (k+1)+" "+aux[j][k]);
                }
     ...
     * @param registro
     */
    public static void verificaRespuesta(int respuesta, int numRondas, Registro registro){
    ...
    /**
     * Metodo que verifica, aplicar este metodo si sabemos que no hay historial por mostrar
     * @param respuesta
     */
